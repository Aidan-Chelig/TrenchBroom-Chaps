...
std::optional<PropertyDefinition> parseListDeclaration(
  ...
  if (expectAttribute(element, "name", status))
  {
    ...
    while (itemElement)
    {
        ...
        && expectAttribute(*itemElement, "value", status))
      {
        ...
        options.push_back(
          PropertyValueTypes::ChoiceOption{std::move(itemValue), std::move(itemName)});
      }
      itemElement = itemElement->NextSiblingElement("item");
    }
    return PropertyDefinition{
      std::move(name), PropertyValueTypes::Choice{std::move(options)}, "", ""};
  }
  return std::nullopt;
}

std::optional<PropertyDefinition> parsePropertyDeclaration(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
...
using PropertyDefinitionFactory =
  std::function<std::optional<PropertyDefinition>(std::string, std::string, std::string)>;

std::optional<PropertyDefinition> parsePropertyDefinition(
  const tinyxml2::XMLElement& element,
...
auto withDefaultValue(
  const tinyxml2::XMLElement& element, const PropertyValueType& valueType)
{
  return std::visit(
    kdl::overload(
      [](const PropertyValueTypes::LinkTarget& targetSourceValueType)
        -> PropertyValueType { return targetSourceValueType; },
      [](const PropertyValueTypes::LinkSource& targetDestinationValueType)
        -> PropertyValueType { return targetDestinationValueType; },
      [&](PropertyValueTypes::String stringValueType) -> PropertyValueType {
      ...
      [&](PropertyValueTypes::Integer integerValueType) -> PropertyValueType {
        if (hasAttribute(element, "value"))
        {
      ...
      [&](PropertyValueTypes::Float floatValueType) -> PropertyValueType {
        if (hasAttribute(element, "value"))
        {
      ...
      [&](PropertyValueTypes::Choice choiceValueType) -> PropertyValueType {
        if (hasAttribute(element, "value"))
        {
      ...
      [&](PropertyValueTypes::Flags flagValueType) -> PropertyValueType {
        if (hasAttribute(element, "value"))
        {
      ...
      [&](PropertyValueTypes::Origin originValueType) -> PropertyValueType {
        if (hasAttribute(element, "value"))
        {
      ...
      [](PropertyValueTypes::Input inputValueType) -> PropertyValueType {
        return inputValueType;
      },
      [](PropertyValueTypes::Output outputValueType) -> PropertyValueType {
        return outputValueType;
      },
      [&]<typename T>(PropertyValueTypes::Color<T> colorValueType) -> PropertyValueType {
        if (hasAttribute(element, "value"))
        {
      ...
      [&](PropertyValueTypes::Unknown unknownValueType) -> PropertyValueType {
        if (hasAttribute(element, "value"))
        {
...
std::optional<PropertyDefinition> parseDeclaredPropertyDefinition(
  const tinyxml2::XMLElement& element,
  const PropertyDefinition& propertyDeclaration,
  ParserStatus& status)
{
...
std::optional<PropertyDefinition> parseTargetNamePropertyDefinition(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
...
std::optional<PropertyDefinition> parseTargetPropertyDefinition(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
  auto factory = [](std::string name, std::string shortDesc, std::string longDesc) {
    return PropertyDefinition{
...
std::optional<PropertyDefinition> parseRealPropertyDefinition(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
  auto factory = [&](std::string key, std::string shortDesc, std::string longDesc) {
    if (hasAttribute(element, "value"))
    {
      if (auto floatDefaultValue = parseFloat(element, "value"))
      {
        return PropertyDefinition{
          std::move(key),
          PropertyValueTypes::Float{floatDefaultValue},
          std::move(shortDesc),
          std::move(longDesc)};
      }
      ...
      return PropertyDefinition{
        std::move(key),
        PropertyValueTypes::Unknown{std::move(strDefaultValue)},
        std::move(shortDesc),
        std::move(longDesc)};
    }
...
std::optional<PropertyDefinition> parseIntegerPropertyDefinition(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
  auto factory = [&](std::string key, std::string shortDesc, std::string longDesc) {
    if (hasAttribute(element, "value"))
    {
      if (auto intDefaultValue = parseInteger(element, "value"))
      {
        return PropertyDefinition{
          std::move(key),
          PropertyValueTypes::Integer{intDefaultValue},
          std::move(shortDesc),
          std::move(longDesc)};
      }
...
std::optional<PropertyDefinition> parseBooleanPropertyDefinition(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
  auto factory = [&](std::string key, std::string shortDesc, std::string longDesc) {
    if (hasAttribute(element, "value"))
    {
      if (auto boolDefaultValue = parseBoolean(element, "value"))
      {
        return PropertyDefinition{
          std::move(key),
          PropertyValueTypes::Boolean{std::move(boolDefaultValue)},
          std::move(shortDesc),
          std::move(longDesc)};
      }
...
std::optional<PropertyDefinition> parseStringPropertyDefinition(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
  auto factory = [&](std::string key, std::string shortDesc, std::string longDesc) {
...
std::optional<PropertyDefinition> parseColorPropertyDefinition(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
  auto factory = [&](std::string key, std::string shortDesc, std::string longDesc) {
    auto defaultValue = hasAttribute(element, "value")
    ...
    return PropertyDefinition{
      std::move(key),
      PropertyValueTypes::Color<Rgb>{std::move(defaultValue)},
      std::move(shortDesc),
      std::move(longDesc)};
  };
...
std::optional<PropertyDefinition> parseUnknownPropertyDefinition(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
  auto factory = [&](std::string key, std::string shortDesc, std::string longDesc) {
    auto defaultValue = hasAttribute(element, "value")
...
std::optional<PropertyDefinition> parsePropertyDefinition(
  const tinyxml2::XMLElement& element,
  const std::vector<PropertyDefinition>& propertyDeclarations,
  ParserStatus& status)
{
...
std::vector<PropertyDefinition> parsePropertyDefinitions(
  ...
  ParserStatus& status)
{
  auto result = std::vector<PropertyDefinition>{};
...
std::optional<PropertyDefinition> parseSpawnflags(
  const tinyxml2::XMLElement& element, ParserStatus& status)
{
  if (const auto* flagElement = element.FirstChildElement("flag"))
  {
    ...
    do
    {
      ...
      else
      {
        ...
        flags.push_back(
          PropertyValueTypes::Flag{value, std::move(shortDesc), std::move(longDesc)});
      }
    ...
    return PropertyDefinition{
      ...
      PropertyValueTypes::Flags{std::move(flags)},
      "",
      ""};
  }
  return std::nullopt;
  ...
  const tinyxml2::XMLDocument& document, ParserStatus& status)
{
  ...
  auto propertyDeclarations = std::vector<PropertyDefinition>{};
